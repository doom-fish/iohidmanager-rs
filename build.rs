...
fn main() {
    ...
    let swift_triple = match env::var("CARGO_CFG_TARGET_ARCH").unwrap_or_default().as_str() {
        "x86_64" => "x86_64-apple-macosx",
        "aarch64" => "arm64-apple-macosx",
        other => panic!(
            "iohidmanager: unsupported target arch '{other}'. Expected x86_64 or aarch64."
        ),
    };
    ...
    println!("cargo:rustc-link-lib=static=IOHIDManagerBridge");
